...
import com.vaadin.starter.bakery.app.security.PasswordEncoderService;
...
public class DataGenerator implements HasLogger {
	...
	private void createOrders(OrderRepository orderRepo, Supplier<Product> productSupplier,
		...
		for (LocalDate dueDate = oldestDate; dueDate.isBefore(newestDate); dueDate = dueDate.plusDays(1)) {
			...
			for (int i = 0; i < ordersThisDay; i++) {
				orderRepo.persist(createOrder(productSupplier, pickupLocationSupplier, barista, baker, dueDate));
			}
		}
	}
	...
	private Supplier<PickupLocation> createPickupLocations(PickupLocationRepository pickupLocationRepository) {
		PickupLocation store = createPickupLocation("Store");
		PickupLocation bakery = createPickupLocation("Bakery");
		pickupLocationRepository.persist(store);
		pickupLocationRepository.persist(bakery);
		List<PickupLocation> pickupLocations = Arrays.asList(store, bakery);
	...
	private User createBaker(UserRepository userRepository, PasswordEncoderService passwordEncoder) {
		User user = createUser("baker@vaadin.com", "Heidi", "Carter", passwordEncoder.encode("baker"), Role.BAKER, false);
		userRepository.persist(user);
		return user;
	}

	private User createBarista(UserRepository userRepository, PasswordEncoderService passwordEncoder) {
		User user = createUser("barista@vaadin.com", "Malin", "Castro", passwordEncoder.encode("barista"), Role.BARISTA, true);
		userRepository.persist(user);
		return user;
	}

	private User createAdmin(UserRepository userRepository, PasswordEncoderService passwordEncoder) {
		User user = createUser("admin@vaadin.com", "Göran", "Rich", passwordEncoder.encode("admin"), Role.ADMIN, true);
		userRepository.persist(user);
		return user;
	}

	private void createDeletableUsers(UserRepository userRepository, PasswordEncoderService passwordEncoder) {
		User peter = createUser("peter@vaadin.com", "Peter", "Bush", passwordEncoder.encode("peter"), Role.BARISTA, false);
		User mary = createUser("mary@vaadin.com", "Mary", "Ocon", passwordEncoder.encode("mary"), Role.BAKER, true);
		userRepository.persist(peter);
		userRepository.persist(mary);
	}
